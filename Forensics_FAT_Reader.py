...
def converttoEntry(FAT_TABLE): # converts a FAT_TABLE to a 12-bits entry
    threeByteEntry = []
    s = 0;

    for x in FAT_Region_1:  # creating a list of bytes
        while 300 > s:
            threeByteEntry.append(FAT_TABLE[s] + FAT_TABLE[s + 1] + FAT_TABLE[s + 2])
            s = s + 3

    entry = []
    for x in threeByteEntry:
        entry.append(x[3] + x[0] + x[1])
        entry.append(x[4] + x[5] + x[2])
    return entry

entryFat1 = []
entryFat2= []
entryFat1= converttoEntry(FAT_Region_1)
entryFat2 = converttoEntry(FAT_Region_2)

for x in range1(0,199): # visualizing from entry 0- 200
     print(str(hexToDec(entryFat1[x])) + "\t" + str(hexToDec(entryFat2[x])))
